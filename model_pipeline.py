...
class ModelPipeline:
    ...
    def create_target(self, df: pd.DataFrame, lookahead: int = 5, threshold: float = 0.002) -> pd.DataFrame:
        ...
        # Calculate forward returns (price lookahead periods in the future)
        ...
        # Drop the forward_return column (no longer needed and contains future info)
        df_target = df_target.drop(columns=['forward_return'])
        
        # Remove lookahead rows (they don't have valid targets)
